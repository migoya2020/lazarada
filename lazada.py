...
res1 = requests.request("GET", x5sec_url, headers=headers1)
...
newCookie2 =cust_cookie2+"_tb_token_="+tb_token+";"+"anon_uid="+anon_id+";"+"lzd_sid="+lzd_sid+";"+"t_fv="+str(getCurrentEpochTime())+";"+"Domain=.lazada.sg; Path=/;"+x5sec
...
headers2.update({"cookie":newCookie2})


final_response = requests.request("GET", url, headers=headers2, data=payload)
...
for script_tag in scripts:
    if "window.pageData" in str(script_tag):
        pageData = str(script_tag).replace("<script>","").replace("</script>","").replace("window.pageData =","").rsplit(";",1)[0].strip()
...
for  item in listItems:
    ...
    try:
        ...
        discount =0
    data.append({"prod_Name":item["name"], "prod_Id":item["nid"],"prod_url":item["productUrl"],"image":item["image"],"priceShow":item["priceShow"], "originalPrice": originalPrice, "discount":discount })
